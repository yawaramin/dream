...
let list_comments =
  ...
  fun (module Db : DB) ->
    ...
    Caqti_eio.or_fail comments_or_error
...
let add_comment =
  ...
  fun text (module Db : DB) ->
    let unit_or_error = Db.exec query text in
    Caqti_eio.or_fail unit_or_error
...
let () =
  ...
  Dream.run env
  @@ Dream.logger
  @@ Dream.sql_pool ~stdenv:(env :> Caqti_eio.stdenv) "sqlite3:db.sqlite"
  @@ Dream.sql_sessions
  @@ Dream.router [
    ...
    Dream.post "/" (fun request ->
      ...
      | `Ok ["text", text] ->
        let () = Dream.sql request (add_comment text) in
